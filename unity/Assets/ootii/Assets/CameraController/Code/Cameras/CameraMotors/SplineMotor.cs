...
namespace com.ootii.Cameras
{
    ...
    public class SplineMotor : CameraMotor
    {
        ...
        /// <summary>
        /// Used to reset the spline motor if we transition out of it with another motor
        /// </summary>
        public override void PostRigLateUpdate()
        {
            base.PostRigLateUpdate();

            if (mHasStarted && RigController.ActiveMotor != this && RigController.ActiveMotor is TransitionMotor)
            {
                TransitionMotor lTransitionMotor = RigController.ActiveMotor as TransitionMotor;
                if (lTransitionMotor.StartMotor == this && lTransitionMotor.TransitionElapsedTime >= lTransitionMotor.TransitionTime)
                {
                    Stop();
                    mDistanceTravelled = 0f;
                }
            }
        }
