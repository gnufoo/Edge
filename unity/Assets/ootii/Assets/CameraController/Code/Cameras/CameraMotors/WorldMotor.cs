    ...
    public class WorldMotor : YawPitchMotor
    {
        ...
        public override CameraTransform RigLateUpdate(float rDeltaTime, int rUpdateIndex, float rTiltAngle = 0f)
        {
            ...
            if (_EdgePan && lMovement.sqrMagnitude == 0f)
            {
                ...
                Vector2 lMousePosition = Vector3.zero;
                UnityEngine.InputSystem.Controls.Vector2Control lMousePositionControl = UnityEngine.InputSystem.Mouse.current.position;
                if (lMousePositionControl != null) { lMousePosition = lMousePositionControl.ReadValue(); }
#else
