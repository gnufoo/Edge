...
namespace com.ootii.Cameras
{
    ...
    public class CameraController : BaseCameraRig
    {
            ...
            if (_IsZoomEnabled)
            {
                if (_ZoomActionAlias.Length > 0)
                {
                    ...
                    else
                    {
                        ...
                        // TRT 10/31/2020: I'm not sure why I was blocking this during a collision. So, I'm going to remove it.
                        //mTargetFOV = Mathf.Clamp(mTargetFOV + lFrameFieldOfView, (mHadCollided ? mOriginalFOV : _ZoomMin), lZoomMax);
                        mTargetFOV = Mathf.Clamp(mTargetFOV + lFrameFieldOfView, _ZoomMin, lZoomMax);
                    }
                }
            }
        ...
        protected virtual void UpdateFade(float rDeltaTime)
        {
            ...
            if (mAlpha != mAlphaEnd)
            {
                mAlphaElapsed = mAlphaElapsed + rDeltaTime;
